...
return        `<rect x="${x}" y="${y}" width="${barW}" height="${Math.max(barH, 2)}" rx="4" fill="${color}" />       <text x="${x + barW / 2}" y="${y - 4}" font-size="11" font-weight="600" fill="#1d1d1f" text-anchor="middle">${sc.pct}%</text>       ${legacyMark}       <text x="${x + barW / 2}" y="${H - 8}" font-size="9" fill="#6e6e73" text-anchor="middle">${h.date.slice(5)}</text>`;
}).join('');

return `<svg viewBox="0 0 ${W} ${H}" width="100%" height="${H}" xmlns="<http://www.w3.org/2000/svg>" style="max-width:${W}px">     <line x1="0" y1="${padTop + usableH}" x2="${W}" y2="${padTop + usableH}" stroke="#e5e5ea" stroke-width="1" />     ${bars}   </svg>`;
}
...
return `<!DOCTYPE html> <html lang="es"> <head> <meta charset="UTF-8"> <meta name="viewport" content="width=device-width,initial-scale=1"> <title>Constructor.io — Historial · Cencosud Chile</title> <style>   *{box-sizing:border-box;margin:0;padding:0}   body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f5f5f7;color:#1d1d1f;padding:2rem}   h1{font-size:20px;font-weight:600;margin-bottom:4px}   .subtitle{font-size:13px;color:#6e6e73;margin-bottom:1.5rem}   .tabs{display:flex;gap:8px;margin-bottom:1.5rem;flex-wrap:wrap}   .tab-btn{background:#fff;border:1px solid #e5e5ea;color:#1d1d1f;padding:8px 16px;border-radius:99px;cursor:pointer;font-size:13px;font-weight:500}   .tab-btn.active{background:#1d1d1f;color:#fff;border-color:#1d1d1f}   .site-card{display:none;background:#fff;border-radius:12px;border:1px solid #e5e5ea;padding:1.25rem;margin-bottom:1rem}   .site-card.active{display:block}   .site-header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:14px}   .site-name{font-size:16px;font-weight:600}   .site-meta{font-size:12px;color:#6e6e73;margin-top:2px}   .site-score{font-size:20px;font-weight:700;padding:6px 14px;border-radius:8px}   .site-score.ok{background:#d4edda;color:#155724}   .site-score.warn{background:#fff3cd;color:#856404}   .site-score.err{background:#f8d7da;color:#721c24}   .chart-wrap{margin-bottom:1.25rem}   .event-table{width:100%;border-collapse:collapse;font-size:12px}   .event-table th{text-align:left;padding:6px 10px;border-bottom:1px solid #e5e5ea;vertical-align:bottom}   .event-table td{padding:7px 10px;border-bottom:1px solid #f5f5f7;vertical-align:middle;white-space:nowrap}   .section-header td{font-size:11px;font-weight:600;color:#6e6e73;text-transform:uppercase;letter-spacing:.05em;background:#f5f5f7;padding:6px 10px}   .tag-label{font-family:monospace;font-size:11px;color:#1d1d1f}   td.ok{color:#155724;font-weight:500}   td.err{color:#721c24;font-weight:500}   td.neutral{color:#aeaeb2}   .date-score{font-size:13px;font-weight:700;border-radius:6px;padding:2px 8px;display:inline-block}   .date-score.ok{background:#d4edda;color:#155724}   .date-score.warn{background:#fff3cd;color:#856404}   .date-score.err{background:#f8d7da;color:#721c24}   .date-label{font-size:10px;color:#6e6e73;margin-top:2px;font-weight:400}   .legacy-tag{display:inline-block;background:#e2e3e5;color:#41464b;font-size:9px;padding:1px 5px;border-radius:4px;margin-left:4px;text-transform:uppercase}   .footer{font-size:11px;color:#aeaeb2;margin-top:1rem}   .legend{font-size:11px;color:#6e6e73;margin-bottom:1rem} </style> </head> <body> <h1>Constructor.io — Historial de Monitoreo · Cencosud Chile</h1> <div class="subtitle">Última actualización: <strong>${date}</strong> · Automatizado vía GitHub Actions (cron diario)</div> <div class="legend">OK · MISSING (en rojo) · N/A no aplica · N/V no verificable por scraping pasivo · — sin dato ese día · <span class="legacy-tag">hist.</span> = carga manual previa, no automatizada</div> <div class="tabs">${tabs}</div> ${siteCards} <div class="footer">Generado automáticamente · Cencosud Chile · Constructor.io tag monitoring</div> <script>   document.querySelectorAll('.tab-btn').forEach(btn => {     btn.addEventListener('click', () => {       document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));       document.querySelectorAll('.site-card').forEach(s => s.classList.remove('active'));       btn.classList.add('active');       document.getElementById(btn.dataset.target).classList.add('active');     });   });   document.querySelector('.site-card').classList.add('active'); </script> </body> </html>`;
}
